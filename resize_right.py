...
class Resizer(nn.Module):
    ...
    def __init__(self,
        ...
        method, kernel_width = {
            ...
            "box": (_box, 1.0),
            "linear": (_linear, 2.0),
            None: (_cubic, 6.0)  # set default interpolation method as cubic
        }.get(kernel)
        ...
        for dim in self.sorted_dims:
            ...
            weights, field_of_view = self._contributions(
                in_shape[dim], output_shape[dim], scale_factor[dim], method,
            ...
            weights_list.append(
                nn.Parameter(torch.reshape(
                    weights,
                    list(weights.shape) + (len(scale_factor) - 1) * [1]),
                             requires_grad=False))
            field_of_view_list.append(
                nn.Parameter(torch.tensor(field_of_view.T.astype(np.int32),
                                          dtype=torch.long),
                             requires_grad=False))
    ...
    def forward(self, in_tensor):
        ...
        assert list(self.in_shape[1:]) == list(
            x.shape[1:]), 'wrong input shape: %s, expected %s' % (str(
                x.shape), str(self.in_shape))

        # Use the affecting position values and the set of weights to calculate the result of resizing along this 1 dim
        for dim, fov, w in zip(self.sorted_dims, self.field_of_view,
                               self.weights):
    ...
    def _fix_scale_and_size(self, input_shape, output_shape, scale_factor):
        ...
        if scale_factor is not None:
            ...
            scale_factor = [1] * (len(input_shape) -
                                  len(scale_factor)) + scale_factor
    ...
    def _contributions(self, in_length, out_length, scale, kernel,
                       kernel_width, antialiasing):
        ...
        fixed_kernel = (lambda arg: scale * kernel(scale * arg)
                        ) if antialiasing and scale < 1.0 else kernel
        ...
        shifted_out_coordinates = out_coordinates - (out_length -
                                                     in_length * scale) / 2
        ...
        match_coordinates = shifted_out_coordinates / scale + 0.5 * (1 -
                                                                     1 / scale)
        ...
        # 'field_of_view')
        weights = fixed_kernel(1.0 *
                               np.expand_dims(match_coordinates, axis=1) -
                               field_of_view - 1)
...
def resize(image: torch.Tensor, *args, **kwargs) -> torch.Tensor:
    resizer = Resizer(image.shape, *args, **kwargs).to(device=image.device)
...
def _lanczos2(x):
    return (((np.sin(math.pi * x) * np.sin(math.pi * x / 2) +
...
def _lanczos3(x):
    return (((np.sin(math.pi * x) * np.sin(math.pi * x / 3) +
              np.finfo(np.float32).eps) /
