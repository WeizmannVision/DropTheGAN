...
def unfold2d(input, kernel_size=7, stride=1, use_padding=True, use_view=False):
    # input dimensions: n, c, h, w
    # kernel: kh, kw
    # output dimensions: n, c, kh, kw, h', w'
...
def fold2d(input, stride=1, use_padding=True, *, reduce='sum', std=1.7):  # noqa
    # input dimensions: n, c, kh, kw, h', w'
    # output dimensions: n, c, h, w
    if input.dim() != 6:
...
def view_as_column(input):
    n, c, kh, kw, h, w = size = input.size()
    output = input.view(n, c * kh * kw, h * w)
    return output, size


def view_as_image(input, size):
